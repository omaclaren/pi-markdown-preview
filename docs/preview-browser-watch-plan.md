...
Only one watcher can run in a pi session. Repeating the same source reopens it; attempting to start a different file or switch between file and response modes reports the active watcher and requires `/preview-browser --stop` first. Browser previews launched from a detected cmux session use a focused cmux browser split in the caller workspace, with automatic system-browser fallback. Multiple named watchers and combined file/response mode remain deferred until there is real demand.
